...
const Produzione: React.FC<{ profile: UserProfile | null }> = ({ profile }) => {
  ...
  // Nello screenshot: "Attività" contiene i "PREP" (PRE) e i "PRO" (PROD)
  ...
  // Nello screenshot: "Coda Attesa" contiene gli "ATT"
    ...
    <div className="min-h-screen bg-slate-100 flex flex-col pb-20 overflow-x-hidden">
      <div className="w-full max-w-[1700px] mx-auto p-4 flex flex-col gap-6">
        <div className="bg-white p-4 rounded-3xl shadow-sm border border-slate-200 flex flex-col md:flex-row justify-between items-center gap-4">
          <div className="flex items-center gap-4">
             ...
             <div>
                ...
                <div className="flex items-center gap-2">
                   {isOperatore ? (
                     <span className="bg-emerald-600 text-white text-[8px] font-black px-2 py-0.5 rounded uppercase flex items-center gap-1 shadow-md"><ShieldCheck size={8} /> OPERATORE</span>
                   ) : (
                     <span className="bg-blue-600 text-white text-[8px] font-black px-2 py-0.5 rounded uppercase flex items-center gap-1 shadow-md"><Eye size={8} /> VISUALIZZATORE</span>
                   )}
          ...
          <div className="flex gap-2">
             <button onClick={() => isOperatore && setShowMagazzinoPicker(true)} disabled={!isOperatore} className={`px-6 py-3 rounded-xl font-black text-[11px] uppercase tracking-widest flex items-center gap-2 transition-all ${isOperatore ? 'bg-slate-950 text-white shadow-xl hover:scale-95' : 'bg-slate-200 text-slate-400 opacity-50 cursor-not-allowed'}`}><Plus size={16}/> PRELEVA</button>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-6 items-start">
          ...
          <div className="bg-[#1e293b] rounded-[2.5rem] shadow-2xl p-6 space-y-4 min-h-[500px]">
             <h3 className="text-white font-black uppercase tracking-widest text-[10px] mb-4 flex justify-between items-center">
             ...
             {proItems.map(l => (
                <div key={l.id_lavorazione} className={`p-6 rounded-[1.8rem] flex items-center justify-between shadow-lg transition-all border-l-[12px] ${l.id_stato === Stati.PRO ? 'bg-sky-500 border-sky-800' : 'bg-white border-slate-300'}`}>
                   <div>
                      ...
                      <p className={`text-[11px] font-black uppercase mt-1 truncate max-w-[200px] tracking-tight ${l.id_stato === Stati.PRO ? 'text-white/80' : 'text-slate-500'}`}>{l.l_clienti?.cliente}</p>
                   </div>
                      ...
                      <div className="flex gap-2">
                         {isOperatore ? (
                           <>
                             ...
                             {l.id_stato === Stati.PRO && <button onClick={() => updateStatoLavorazione(l.id_lavorazione, Stati.TER)} className="px-6 py-3 bg-white text-blue-900 rounded-xl font-black text-[10px] uppercase border-2 border-blue-800 shadow-xl active:scale-95 transition-all">TERMINA</button>}
                           </>
                         ) : <div className="p-4 bg-slate-100 text-slate-300 rounded-2xl"><Lock size={18} /></div>}
                      </div>
                   </div>
                </div>
             ))}
          ...
          <div className="bg-[#f29f05] rounded-[2.5rem] shadow-2xl p-6 space-y-4 min-h-[500px]">
             ...
             {attItems.map(l => (
                <div key={l.id_lavorazione} className="bg-[#ffa500] p-6 rounded-[1.8rem] border-l-[12px] border-[#cc8400] flex items-center justify-between shadow-xl">
                   <div>
                      <h4 className="font-black text-4xl italic leading-none text-slate-950">{l.scheda}</h4>
                      <p className="text-[11px] font-black uppercase mt-1 text-amber-950 truncate max-w-[200px]">{l.l_clienti?.cliente}</p>
                   </div>
                   <div className="flex items-center gap-8">
                      <div className="text-right">
        ...
        <div className="fixed inset-0 bg-slate-950/90 backdrop-blur-md flex items-center justify-center p-4 z-[200]">
           <div className="bg-white rounded-[3rem] w-full max-w-2xl shadow-2xl flex flex-col max-h-[85vh] overflow-hidden">
              <div className="bg-slate-950 p-8 text-white flex justify-between items-center">
                 <div className="flex items-center gap-4"><Inbox size={24}/><h3 className="font-black text-2xl uppercase italic tracking-tighter">Magazzino</h3></div>
                 <button onClick={() => setShowMagazzinoPicker(false)} className="hover:rotate-90 transition-transform"><X size={32}/></button>
              </div>
              <div className="p-8 overflow-y-auto space-y-4 bg-slate-50 flex-grow">
                 {magazzino.map(s => (
                    <button key={s.id_lavorazione} onClick={async () => { 
                       await supabase.from('l_lavorazioni').update({ id_macchina: selectedMacchina, ultimo_utente_id: profile?.id }).eq('id_lavorazione', s.id_lavorazione);
                       ...
                       <div className="p-4 bg-slate-50 text-blue-600 rounded-2xl group-hover:bg-blue-600 group-hover:text-white transition-all"><Plus size={20} /></div>
                    </button>
                 ))}
              </div>
           </div>
      ...
      {showFasePicker && (
         <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-sm flex items-center justify-center z-[1000] p-4">
            <div className="bg-white p-10 rounded-[2.5rem] w-full max-w-xs text-center shadow-2xl">
               <h4 className="font-black uppercase tracking-widest text-[10px] text-slate-400 mb-8 italic">Seleziona Tipo Lavorazione</h4>
               <div className="flex flex-col gap-3">
                  {fasi.filter(f => f.id_fase !== 'ATT').map(f => (
                     <button key={f.id_fase} onClick={() => startLavorazione(showFasePicker.id, f.id_fase)} className="p-5 bg-slate-50 border border-slate-100 rounded-2xl font-black uppercase text-[11px] hover:bg-blue-600 hover:text-white hover:shadow-xl transition-all flex justify-between items-center">
                        <span>{f.fase_di_lavorazione}</span><ArrowRight size={14} />
                     </button>
                  ))}
               </div>
            </div>
      ...
      {showMacchinaPicker && (
        <div className="fixed inset-0 bg-slate-950/95 backdrop-blur-xl flex items-center justify-center p-4 z-[2000]">
           <div className="bg-white rounded-[3rem] p-10 w-full max-w-sm shadow-2xl text-center">
              ...
              <button onClick={() => setShowMacchinaPicker(false)} className="text-[10px] font-black uppercase text-slate-400">Annulla</button>
           </div>
        </div>
      )}

      <Chat />
      {loading && <div className="fixed inset-0 bg-slate-950/90 flex flex-col items-center justify-center z-[9999]"><RefreshCw className="text-blue-600 animate-spin mb-4" size={48} /><p className="text-white text-[10px] font-black uppercase tracking-widest">Sincronizzazione...</p></div>}
    </div>
  );
};
